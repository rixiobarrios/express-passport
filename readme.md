...
"_Passport is authentication Middleware for Node. It is designed to serve a
singular purpose: authenticate requests. When writing modules, encapsulation is
a virtue, so Passport delegates all other functionality to the application. This
separation of concerns keeps code clean and maintainable, and makes Passport
extremely easy to integrate into an application._

_In modern web applications, authentication can take a variety of forms.
Traditionally, users log in by providing a username and password. With the rise
of social networking, single sign-on using an OAuth provider such as Facebook or
Twitter has become a popular authentication method. Services that expose an API
often require token-based credentials to protect access._"
...
```bash
$ git clone git@git.generalassemb.ly:dc-wdi-node-express/express-passport-local-authentication.git
```

`cd` into the cloned folder, and run `npm install` to ensure that we have all of
...
In `app.js` require in passport. Near the top of your code add in
`const passport = require('passport')`

Just below all of the requires at the top, add in:
...
Take a few minutes to look over the files and familiarize yourself with the
content and code structure. What looks familiar? What's new? What stands out?
...
This will generate a salt token and then hash the password using this new salt.

Then we use it back on the signup strategy:

```javascript
let newUser = new User()
newUser.email = email
// instead of setting the value directly like this
// newUser.password = password

// we call the encrypt method we just defined and pass it the password
...
## Flash Messages (10 min / 1:20)

Flash messages are one-time messages that are rendered in the views, and when
the page was reloaded, the flash is destroyed. If we've configured express correctly, we can pass messages back to the view depending on whether the request was successful or not.
...
`config/passport.js`. These should go inside the `passportConfig` function, after `passport.use()`.
...
`config/passport.js`, after the signup strategy (before the serialize methods), add add a new LocalStrategy:
...
Inside the `local-login` strategy callback in `passport.js` let's add this code:
...
```js
// require("./config/passport")(passport)
// below the passport config require, add this
...
<ul class="nav navbar-nav">
  {{#if currentUser}}
    <li><a href="/logout">Logout {{currentUser.email}}</a></li>
  {{else}}
